...
(define (vector3d-inverse v1)
  (make-vector3d (fl- (vector3d-x v1))
                 (fl- (vector3d-y v1))
                 (fl- (vector3d-z v1))))
...
(define (trace-scene port)
  (let* ((eye-unit (vector3d-unit EYE))
         (point-unit (vector3d-unit POINT))
         (eye-right (vector3d-cross eye-unit (make-vector3d 0.0 1.0 0.0)))
         (eye-up (vector3d-cross eye-unit eye-right))
         (dX (vector3d-scalar-mul eye-right (fl/ VIEWPORT-X (integer->float WINDOW-X))))
         (dY (vector3d-scalar-mul eye-up (fl/ VIEWPORT-Y (integer->float WINDOW-Y))))
         (corner (vector3d-add (vector3d-scalar-mul (vector3d-inverse eye-right) (fl/ VIEWPORT-X 2.0))
                               (vector3d-scalar-mul (vector3d-inverse eye-up) (fl/ VIEWPORT-Y 2.0))))
         (count (* WINDOW-X WINDOW-Y)))
    (let loop ((n 0))
      (if (< n count)
          (let* ((x (remainder n WINDOW-X))
                 (y (quotient n WINDOW-X))
                 (point (vector3d-add (vector3d-scalar-mul dX (integer->float x))
                                      (vector3d-scalar-mul dY (integer->float y))))
                 (view (vector3d-unit
                        (vector3d-sub (vector3d-add corner point) POINT)))
                 (color (shoot-ray POINT view 0)))
...
(define (generate-sphere-plane objects start)
  (let loop ((i 0))
    (if (< i 25)
        (begin
          (vector-set! objects (+ start i)
                       (make-sphere (make-vector3d .2 .5 .5)
                                    (make-vector3d (fl* 15.0 (integer->float (remainder i 5)))
                                                   0.0
                                                   (fl+ (fl* 15.0 (integer->float (quotient i 5)))
                                                        15.0))
                                    5.0))
          (loop (+ i 1))))))

(define (configure)
  (set! EYE (make-vector3d 0.0 0.0 1.0))
  (set! POINT (make-vector3d 0.0 0.0 -5.0))
  ...
  (let* ((count 27)
    ...
    (vector-set! objects 1 (make-plane (make-vector3d .7 .7 1.0)
                                       (make-vector3d 0.0 -1.0 0.0)
                                       5.0))
    (generate-sphere-plane objects 2)
    (set! SCENE objects)
