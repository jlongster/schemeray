    ...
    (let ((vector (cadr f))
      ...
      `(let loop ((,i 0))
         (if (< ,i (vector-length ,vector))
             (let ((,binding (vector-ref ,vector ,i)))
               ,body
               (loop (+ ,i 1))))))))
...
(define (vector3d-unit v1)
  (vector3d-scalar-mul v1 (fl/ (vector3d-length v1))))
...
(define (sphere-intersection obj origin dir)
  (let* ((eo (vector3d-sub origin (sphere-position obj)))
         ...
         (c (fl- (vector3d-dot eo eo) (flexpt (sphere-radius obj) 2.0)))
         (d (fl- (fl* b b) c)))
    (if (fl<= d 0.0)
        DEPTH
...
(define (find-closest-prim orig dir prims succeed fail)
  (set! fcp-obj #f)
  (set! fcp-depth DEPTH)
  (vector-iterate prims o
    (let ((d (object-intersection o orig dir)))
      (if (fl< d depth)
          (begin
            ...
            (set! fcp-depth d)))))
  (if fcp-obj
      (succeed fcp-obj fcp-depth)
      (fail)))

(define (shoot-ray orig dir step)
  (if (< step NUM-STEPS)
      (find-closest-prim orig dir SCENE
                         (lambda (obj depth)
                           (let* ((point (vector3d-add orig (vector3d-scalar-mul dir depth)))
                             ...
                             (vector3d-add
                              ...
                              (vector3d-scalar-mul
                               (shoot-ray (vector3d-add point (vector3d-scalar-mul r .0001))
                                          r
                                          (+ step 1))
                               (fl/ (fl* (integer->float (+ step 1)) 2.0))))))
                         (lambda ()
                           DEFAULT-COLOR))
    ...
    (let loop ((n 0))
      (if (fl< n count)
          (let* ((point (make-vector3d (fl* (integer->float (remainder n WINDOW-X)) dX)
                                       ...
                                       0.0))
                 (view (vector3d-unit
                        (vector3d-sub (vector3d-add corner point))))
